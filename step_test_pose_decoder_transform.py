...
def evaluate_alignment(src_pts: np.ndarray, tgt_pts: np.ndarray, thresh=0.015):
    ...
    # 综合排序分
    score = (
        0.55 * fit15_tgt_to_src +
        0.45 * fit15_src_to_tgt
        - 0.25 * rmse_tgt_to_src
        - 0.15 * rmse_src_to_tgt
    )
...
def apply_pose_transform(full_pts: np.ndarray, scale: np.ndarray, R: np.ndarray, t: np.ndarray):
    # pose decoder 现在输出的是 3 维 scale
    scale = np.asarray(scale, dtype=np.float64).reshape(1, 3)
...
def main():
    ...
    parser.add_argument("--full", required=True, help="canonical 完整点云/网格，例如 sam3d_duck_clean.ply")
    parser.add_argument("--sam-partial", required=True, help="SAM pointmap 导出的 sam_partial_rgb.ply")
    ...
    print("\n" + "=" * 60)
    print("[🚀] 启动：测试 pose decoder 的 scale+rotation+translation 是否可直接恢复 RGB 姿态")
    print("=" * 60)
    ...
    if "rotation_quat" in pose_data:
        raw_rot = pose_data["rotation_quat"]
    elif "rotation" in pose_data:
        raw_rot = pose_data["rotation"]
    else:
        raise RuntimeError("pose json 里没有 rotation / rotation_quat")

    if "translation" not in pose_data or "scale" not in pose_data:
        raise RuntimeError("pose json 里缺少 translation 或 scale")
    ...
    print("[4] 正在测试 4 种 pose 解释...\n")

    for name, R in candidates:
        full_try = apply_pose_transform(full_pts, s, R, t)
        ...
        save_point_cloud(os.path.join(name_dir, "full_transformed.ply"), full_try, color=[0.10, 0.45, 0.85])
        save_point_cloud(os.path.join(name_dir, "sam_partial_rgb.ply"), sam_partial_pts, color=[0.85, 0.10, 0.10])
        save_merged_point_cloud(os.path.join(name_dir, "merged_full_vs_sam_partial.ply"), full_try, sam_partial_pts)
    ...
    result_json = os.path.join(args.out_dir, "pose_decoder_test_results.json")
    with open(result_json, "w", encoding="utf-8") as f:
        json.dump(all_results, f, indent=2, ensure_ascii=False)

    if best is None:
        raise RuntimeError("没有得到有效候选。")

    best_full_path = os.path.join(args.out_dir, "best_full_transformed.ply")
    best_merge_path = os.path.join(args.out_dir, "best_merged_full_vs_sam_partial.ply")
    ...
    best_json = os.path.join(args.out_dir, "best_pose_decoder_candidate.json")
